...
@pytest.mark.parametrize(
    "number, expected",
    [
        ("Visa Gold", "Неправильно введены данные!"),
...
def test_mask_account_card_incorrect(number: str, expected: str) -> None:
    assert mask_account_card(number) == expected


def test_mask_account_card_incorrect_5() -> None:
    with pytest.raises(TypeError):
        mask_account_card()


def test_get_date_correct_date(correct_date: str) -> None:
...
@pytest.mark.parametrize(
    "date, expected",
    [
        ("2024-03-11T02:26:18.6714070", "Неправильная длина данных!"),
...
def test_get_date_incorrect_date(date: str, expected: str) -> None:
    with pytest.raises(ValueError) as exc_info:
        get_date(date)
    assert str(exc_info.value) == expected


def test_get_date_incorrect_date_8() -> None:
    with pytest.raises(TypeError):
        get_date()
